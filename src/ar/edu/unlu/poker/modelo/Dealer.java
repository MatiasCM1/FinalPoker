...
public class Dealer {
	...
	public Jugador devolverGanador(Queue<Jugador> ronda) throws RemoteException {
		...
		for (Jugador jugadorActual : this.rondaApuestaAux) {
			...
			if (jugadorActualMayorJugadorGanador(jugadorGanador, jugadorActual)) { // Si el actual es mayor, lo pongo
																					// como el nuevo ganador

				ganadores.clear();
				ganadores.add(jugadorActual);
				jugadorGanador = jugadorActual;

			} else if (jugadorActualIgualJugadorGanador(jugadorGanador, jugadorActual)) { // si el actual es igual, lo
																							// agrego a la lista de
																							// ganadores
...
//-------------------------------------------------------------------------------------
	...
	private Jugador determinarGanador(Queue<Jugador> ganadores) throws RemoteException {
		...
		// Si el resultado empatado es carta mayor, color, escalera o escalera a color
		// se busca la carta mayor
		if (resultadoEsCartaMayorColorEscaleraEscaleraColor(resultadoGanador)) {

			return buscarJugadorCartaMayor(ganadores);

		} else if (resultadoGanador.equals(Resultado.PAR)) { // Si el resultado empatado es par, gana el que tiene el
																// par mayor
	...
	private Jugador buscarJugadorCartaMayor(Queue<Jugador> ganadores) {
		...
		for (Jugador jugadorActual : ganadores) {
			...
			if (cartaActual.compareTo(cartaMayor) > 0) { // Si la carta mayor del jugador actual es mayor a la carta
															// mayor, la reemplazo
	...
	private Jugador devolverGanadorResultadosCartasIguales(Queue<Jugador> ganadores) {
		...
		for (Jugador jugadorActual : ganadores) {
			if (cartasIgualadasMayores(ganador, jugadorActual)) { // Si las cartas igualadas del jugador actual son
																	// mayores, entonces pasa a ser el ganador
	...
	private boolean cartasIgualadasMayores(Jugador ganadorActual, Jugador candidadatoAGanador) {
		...
		// Paso una lista con las cartas que son iguales de cada jugador, esta lista se
		// ordena de menor a mayor y tomo la ultima, ya que esta seria la mayor
		Carta cartaMayor1 = (ganadorActual.ordenarCartas(ganadorActual.determinarCartasIguales())).getLast();
		Carta cartaMayor2 = (candidadatoAGanador.ordenarCartas(candidadatoAGanador.determinarCartasIguales()))
				.getLast();

		// Devuelve true si la carta (que forma parte del resultado de cartas igualadas)
		// de candidatoAGanador es mayor a la del ganadorActual
		return cartaMayor2.compareTo(cartaMayor1) >= 0;

	}

	// Dos cartas con el mismo numero, tres cartas con el mismo numero pero distinto
	// de las dos
	private Jugador devolverGanadorResultadosCartasFull(Queue<Jugador> ganadores) {
		...
		for (Jugador jugadorActual : ganadores) {
			if (cartasIgualadasMayoresFull(ganador, jugadorActual)) { // Si el trio de cartas del full del jugador
																		// actual son mayores, entonces pasa a ser el
																		// ganador
				ganador = jugadorActual;
			}
		}
